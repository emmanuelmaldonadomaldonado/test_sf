...
class users(UserMixin, db.Model):
   ...
   email_verified = Column(Boolean, default=False)  # Nuevo campo para verificar el correo electrÃ³nico
   date_created = Column(DateTime, default=datetime.utcnow)
   confirmation_token = Column(String(100), unique=True, nullable=True)  # Campo para almacenar el token de verificaciÃ³n
...
def confirm_email(self, token):
   serializer = URLSafeTimedSerializer(app.secret_key)
   try:
       email = serializer.loads(token, max_age=3600)  # El token es vÃ¡lido por 1 hora (3600 segundos)
   except:
       return False
   if email == self.email:
...
class products(db.Model):
   ...
   user_id = Column(Integer, ForeignKey('users.id'), nullable=False)  # AÃ±adido campo user_id
   user = relationship('users', backref='products')  # RelaciÃ³n con el modelo users
...
def send_verification_email(user_email):
   token = secrets.token_urlsafe(20)  # Genera un token Ãºnico
   ...
   message = Mail(
       ...
       html_content=f'Por favor, haz clic en el siguiente enlace para verificar tu correo electrÃ³nico: <a href="{verification_link}">Verificar correo electrÃ³nico</a>'
   )
   try:
...
def signup():
   if request.method == 'POST':
       ...
       # Verificar si la contraseÃ±a cumple con las restricciones
       if not re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$', password):
           flash('La contraseÃ±a debe tener al menos 8 caracteres, una mayÃºscula, un nÃºmero y un sÃ­mbolo especial.', 'error')
           return redirect('/signup')


       if password != confirm_password:
           flash('Las contraseÃ±as no coinciden. Por favor, intÃ©ntalo de nuevo.', 'error')
           return redirect('/signup')
       ...
       try:
           ...
           flash('Cuenta creada exitosamente. Por favor, verifica tu correo electrÃ³nico.', 'success')
           return redirect('/login')
       except:
           flash('Error al crear la cuenta. Por favor, intÃ©ntalo de nuevo.', 'error')
           return redirect('/signup')
   else:
       return render_template('signup.html')


# Ruta para la confirmaciÃ³n del correo electrÃ³nico
...
def change_password():
   ...
   if not check_password_hash(current_user.password, current_password):
       flash('La contraseÃ±a actual es incorrecta. Por favor, intÃ©ntalo de nuevo.', 'error')
       return redirect('/settings')


   if new_password != confirm_password:
       flash('Las contraseÃ±as nuevas no coinciden. Por favor, intÃ©ntalo de nuevo.', 'error')
       return redirect('/settings')


   if not re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$', new_password):
       flash('La nueva contraseÃ±a debe tener al menos 8 caracteres, una mayÃºscula, un nÃºmero y un sÃ­mbolo especial.', 'error')
       return redirect('/settings')
   ...
   try:
       ...
       flash('Error al cambiar la contraseÃ±a. Por favor, intÃ©ntalo de nuevo.', 'error')
       return redirect('/settings')
...
def update_product(id):
   ...
   if request.method == 'POST':
       ...
       try:
           ...
           # Enviar alerta si la cantidad estÃ¡ por debajo del nivel de alerta
...
def view_items():
   ...
   if search_value:
       ...
       # Si no hay consulta de bÃºsqueda, obtener todos los elementos excluyendo los eliminados
...
def update_quantity(task_id):
   ...
   try:
       ...
       # Verificar si la cantidad estÃ¡ por debajo del umbral de alerta
       if task.alert_quantity is not None and new_quantity <= task.alert_quantity:
           send_alert_email(task)


       # Registrar la actualizaciÃ³n en el historial de productos
       history_entry = productHistory(
           product_id=task.id,
           product_name=task.name,
           quantity=new_quantity,
